...
public class AdminProductController {
	...
	public ResponseEntity<?> addProduct(@RequestBody Map<String, Object> productRequest) {
		try {
			...
			Integer stock = (Integer) productRequest.get("categoryId");
			String imageUrl = (String) productRequest.get("imageUrl");
			
			Product addedProduct = adminProductService.addProductWithImage(name, description, price, stock, stock, imageUrl);
	...
	public ResponseEntity<?> deleteProduct(@RequestBody Map<String, Integer> requestBody) {
		try {
			...
			return ResponseEntity.status(HttpStatus.OK).body("Product deleted successffully");
		} catch (IllegalArgumentException e) {
