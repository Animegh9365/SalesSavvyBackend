...
public class AuthenticationFilter implements Filter { 
	...
	private void executeFilterLogic(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		logger.info("Request URI: {}", requestURI); // Allow unauthenticated paths
	...
	private String getAuthTokenFromCookies(HttpServletRequest request) {
		...
		if (cookies != null) { 
			return Arrays.stream(cookies) .filter(cookie -> "authToken".equals(cookie.getName())) .map(Cookie::getValue) .findFirst() .orElse(null); 
			} 
		return null; 
		} 
